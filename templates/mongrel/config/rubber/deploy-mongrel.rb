  ...
  namespace :mongrel do
    ...
    task :custom_install, :roles => :mongrel do
      ...
      rubber.sudo_script 'setup_mongrel_init', <<-ENDSCRIPT
        ...
        rm -f /etc/mongrel_cluster/#{application}-#{Rubber.env}.yml && ln -s /ebs/#{application}-#{Rubber.env}/current/config/mongrel_cluster.yml /etc/mongrel_cluster/#{application}-#{Rubber.env}.yml
