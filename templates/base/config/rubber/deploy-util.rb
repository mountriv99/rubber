  ...
  namespace :util do
    ...
    task :backup do
      ...
      task task_name, :hosts => selected_db_instance.full_name do
        rsudo "cd #{current_path} && RUBBER_ENV=#{Rubber.env} ./script/rubber util:backup_db --directory=/ebs/db_backups --dbuser=#{rubber_env.db_user} --dbpass=#{rubber_env.db_pass} --dbname=#{rubber_env.db_name} --dbhost=#{selected_db_instance.full_name}"
      end
      send task_name
    end
